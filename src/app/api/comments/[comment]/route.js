  ...
  try {
    ...
    if (!comment) {
      return NextResponse.json(
        { error: "Missing route param: comment" },
        { status: 400 }
      );
    }
    if (!userData?.comment) {
      return NextResponse.json(
        { error: "Missing body field: comment" },
        { status: 400 }
      );
    }

    const newPost = await homepageData.addComment(
      comment,
      userData.comment,
      uid
    );
...
export async function PATCH(req, { params }) {
  try {
    ...
    const newPost = await homepageData.patchComment(
      comment,
      userData.commentId,
      userData.comment,
      uid
