...
export async function GET(request) {
  try {
    ...
    if (useQueue) {
      try {
        console.log(
          `[API] 🔍 Checking RabbitMQ availability for pincode: ${validation.formatted}`
        );
        const rabbitmqAvailable = await isRabbitMQAvailable();
        console.log(
          `[API] 🔍 RabbitMQ available check result: ${rabbitmqAvailable}`
        );

        if (rabbitmqAvailable) {
          console.log(
            `[API] 🐰 Using RabbitMQ for geocoding coords - Pincode: ${validation.formatted}`
          );
          const job = await publishGeocodingJob(validation.formatted, "coords");

          if (job.cached && job.status === "completed") {
            console.log(
              `[API] ✅ Geocoding result found in cache - Pincode: ${validation.formatted} (no RabbitMQ job needed)`
            );
            ...
            console.warn(
              `[API] ⚠️  Cached result not found in Redis for job ${job.jobId}, returning job ID for polling`
            );
          } else {
            console.log(
              `[API] 📤 Geocoding job published to RabbitMQ - Job ID: ${job.jobId}, Pincode: ${validation.formatted}`
            );
          }
          ...
          return NextResponse.json(
            {
              ...
              status: "queued",
              message:
                "Geocoding job queued. Poll /api/geocoding/status/[jobId] for results.",
              pincode: validation.formatted,
            },
            { status: 202 }
          ); // 202 Accepted
          ...
          console.log(
            `[API] ⚠️  RabbitMQ not available (isRabbitMQAvailable returned false) - Pincode: ${validation.formatted}`
          );
        }
      } catch (queueError) {
        console.warn(
          `[API] ⚠️  RabbitMQ unavailable, falling back to direct geocoding - Pincode: ${validation.formatted}`,
          queueError.message
        );
      ...
      console.log(
        `[API] 🔄 Queue disabled (useQueue=false) - Pincode: ${validation.formatted}`
      );
    }

    console.log(
      `[API] 🔄 Using direct geocoding (no RabbitMQ) - Pincode: ${validation.formatted}`
    );
    let coords;
    try {
