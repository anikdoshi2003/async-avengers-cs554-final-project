    ...
    if (useQueue) {
      try {
        ...
        if (rabbitmqAvailable) {
          console.log(
            `[API] 🐰 Using RabbitMQ for reverse geocoding - Lat: ${latNum}, Lng: ${lngNum}`
          );
          ...
          if (job.cached || job.status === "completed") {
            console.log(
              `[API] ✅ Reverse geocoding result found in cache - Lat: ${latNum}, Lng: ${lngNum} (no RabbitMQ job needed)`
            );
          ...
          console.log(
            `[API] 📤 Reverse geocoding job published to RabbitMQ - Job ID: ${job.jobId}, Lat: ${latNum}, Lng: ${lngNum}`
          );
          return NextResponse.json(
            {
              success: true,
              ...
              message:
                "Reverse geocoding job queued. Poll /api/geocoding/status/[jobId] for results.",
              lat: latNum,
              lng: lngNum,
            },
            { status: 202 }
        ...
        console.warn(
          `[API] ⚠️  RabbitMQ unavailable, falling back to direct reverse geocoding - Lat: ${latNum}, Lng: ${lngNum}`,
          queueError.message
        );
      }
    }
